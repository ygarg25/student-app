...
class EditStudent extends Component {
  ...
  handleSubmit = async fields => {
    ...
    let apiEndPoint = "http://localhost:2410/student/" + fields.name;
    const { data: response } = await axios.put(apiEndPoint, fields);
    alert("Status!! :-)\n\n" + JSON.stringify(response, null, 4));
    window.location = "/allMobs";
  };
  ...
  async componentWillMount() {
    ...
    let { data: studentData } = await axios.get(apiEndPoint);
  ...
  render() {
    const { studentData: s, sectionData } = this.state;
